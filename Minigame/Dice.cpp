...
//////////////
*/
// totalScore 출력수정, setDice() 위치수정 start -> selectScore
...
void Dice::drawStatus() { // 게임 중 상태창 출력
    GotoXY(0, 17);
    cout << "┌──────────────────────────────────┐   " << endl;
    cout << "                                   " << endl;
    cout << "                                   " << endl;
...
void Dice::setTotalScore() { // 총점을 계산하여 저장하는 함수
    totalScore = 0;
    for (int i = 0; i < 10; i++) {
        if (score[i] >= 0) {
            totalScore += score[i];
...
int Dice::getScore(int num) { // 각 점수판에 따라 점수를 계산하여 반환하는 함수
    int result = 0;
    switch (num)
    {
    case CHOICE:
...
void Dice::selectScore() { // 점수판에서 enter를 입력했을때 점수를 선택하여 할당하는 함수
    if (score[player.y] < 0) {
...
void Dice::selectDice() { // 주사위를 선택하는 함수
...
int Dice::rollDice() { // 1~6까지 랜덤한 수를 반환하는 함수
...
void Dice::reRoll() { // 선택한 주사위에 다시 랜덤값을 할당하는 함수
    if (reRollCount < 5) {
        for (int i = 0; i < 5; i++) {
            if (select[i] == true) {
                dice[i] = rollDice();
